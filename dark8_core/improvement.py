...
class PromptTuner:
    ...
    def get_improvement_report(self, intent: str) -> Dict:
        ...
        return {
            ...
            "best_score": max(
                self.performance_scores.get(f"{intent}_v{i}", 0)
...
class SelfImprovementEngine:
    ...
    def analyze_and_improve(self, metrics: Dict, context: Dict) -> Dict:
        ...
        recommendation["confidence"] = min(
            metrics.get("success_rate", 0), 1.0 - metrics.get("error_rate", 0)
        )
    ...
    def get_improvement_summary(self) -> Dict:
        ...
        successful = sum(1 for x in self.improvement_history if x.get("confidence", 0) > 0.7)
