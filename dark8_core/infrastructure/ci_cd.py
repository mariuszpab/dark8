...
class PipelineEngine:
    ...
    def create_pipeline(
        self, commit_sha: str, branch: str = "main", triggered_by: str = "webhook"
    ) -> Pipeline:
        """
    ...
    def create_default_jobs(self, pipeline: Pipeline) -> List[PipelineJob]:
        ...
        jobs = [
            ...
            PipelineJob(
                job_id=f"{pipeline.pipeline_id}-test", name="Run tests", stage=PipelineStage.TEST
            ),
            PipelineJob(
...
class ArtifactManager:
    ...
    def store_artifact(
        self, name: str, version: str, type: str, size_bytes: int, metadata: Dict = None
    ) -> bool:
        """
...
class Webhook:
    ...
    def handle_push_event(self, repository: str, branch: str, commit_sha: str) -> bool:
        """
        ...
        pipeline = self.pipeline_engine.create_pipeline(
            commit_sha=commit_sha, branch=branch, triggered_by="webhook"
        )

        # Uruchom pipeline
        return self.pipeline_engine.run_pipeline(pipeline.pipeline_id)

    def handle_pull_request_event(self, repository: str, branch: str, commit_sha: str) -> bool:
        ...
        pipeline = self.pipeline_engine.create_pipeline(
            commit_sha=commit_sha, branch=branch, triggered_by="pull_request"
        )
...
def test_cicd_pipeline():
    ...
    pipeline = engine.create_pipeline(
        commit_sha="abc123def456", branch="main", triggered_by="webhook"
    )
    ...
    result = webhook.handle_push_event("https://github.com/dark8os/core", "main", "xyz789abc123")
