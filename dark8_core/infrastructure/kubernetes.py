...
class Pod:
    ...
    def is_healthy(self) -> bool:
        ...
        return self.status == PodStatus.RUNNING and self.ready_replicas == self.replicas
...
class KubernetesCluster:
    ...
    def create_deployment(
        self, name: str, image: str, replicas: int = 3, cpu: str = "250m", memory: str = "256Mi"
    ) -> bool:
        """
...
class DeploymentManager:
    ...
    def deploy_service(
        self, name: str, image: str, replicas: int = 3, wait_for_ready: bool = True
    ) -> bool:
        """Deployment serwisu z czekaniem na gotowość"""
        success = self.cluster.create_deployment(name=name, image=image, replicas=replicas)
        ...
        if success:
            ...
            self.rollback_points[name].append(
                {"image": image, "replicas": replicas, "timestamp": datetime.now().isoformat()}
            )
    ...
    def rollback_deployment(self, name: str) -> bool:
        ...
        previous = (
            self.rollback_points[name][-2]
            if len(self.rollback_points[name]) > 1
            else self.rollback_points[name][-1]
        )
...
def test_kubernetes_manager():
    ...
    cluster = KubernetesCluster({"context": "dark8-prod", "namespace": "dark8-system"})
