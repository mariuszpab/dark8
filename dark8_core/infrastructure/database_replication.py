...
class DatabaseReplication:
    ...
    def get_slaves(self) -> List[DatabaseInstance]:
        ...
        return [inst for inst in self.instances.values() if inst.role == "slave" and inst.is_active]

    def write_to_binlog(self, query: str, database: str, transaction_id: int) -> BinlogEntry:
        """
        Zapis do binlogu
...
class ConflictResolver:
    ...
    def detect_conflict(
        self, instance_id: str, row_id: str, local_version: int, remote_version: int
    ) -> bool:
        """
        Detekcja konfliktu
    ...
    def resolve_conflict(self, row_id: str, local_data: Dict, remote_data: Dict) -> Dict:
        """
...
class FailoverManager:
    ...
    def automatic_failover(self) -> bool:
        ...
        best_slave = min(
            slaves, key=lambda s: self.replication.replication_lag.get(s.instance_id, float("inf"))
        )
...
def test_database_replication():
    ...
    master = DatabaseInstance(
        instance_id="db-master-1", host="db-master.dark8.local", port=5432, role="master"
    )
    repl.add_instance(master)

    for i in range(2):
        slave = DatabaseInstance(
            instance_id=f"db-slave-{i}", host=f"db-slave-{i}.dark8.local", port=5432, role="slave"
        )
    ...
    for i in range(5):
        entry = repl.write_to_binlog(
            query=f"INSERT INTO users VALUES ({i})", database="dark8_db", transaction_id=i
        )
