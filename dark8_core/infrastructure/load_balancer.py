...
class LoadBalancer:
    ...
    def _select_least_connections(self, backends: List[Backend]) -> Backend:
        ...
        return min(backends, key=lambda b: self.backend_connection_count[b.id])
    ...
    def handle_request(self, client_ip: str) -> Optional[Tuple[str, int]]:
        ...
        connection = Connection(
            client_ip=client_ip, backend_id=backend.id, established_at=datetime.now()
        )
    ...
    def close_connection(
        self, client_ip: str, bytes_sent: int = 0, bytes_received: int = 0
    ) -> None:
...
def test_load_balancer():
    ...
    for i in range(3):
        backend = Backend(
            id=f"backend-{i}", host=f"server-{i}.dark8.local", port=8080 + i, weight=1.0
        )
