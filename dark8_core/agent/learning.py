...
class PatternRecognition:
    ...
    def _load_patterns(self):
        ...
        try:
            ...
            self.patterns = {k: v / 5.0 for k, v in success_count.items()}
...
class PromptOptimizer:
    ...
    def optimize_prompt(self, base_prompt: str, intent: str, success_rate: float) -> str:
...
class AgentLearner:
    ...
    def get_success_rate(self, intent: str, time_window_hours: int = 24) -> float:
        ...
        recent_metrics = [m for m in self.metrics if m.intent == intent and m.timestamp > _cutoff]
        ...
        for intent, intent_metrics in by_intent.items():
            ...
            knowledge[intent] = {
                ...
                "status": (
                    "solid"
                    if success_rate > 0.8
                    else "improving" if success_rate > 0.5 else "needs work"
                ),
            }
